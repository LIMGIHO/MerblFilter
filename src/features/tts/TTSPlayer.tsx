...
function cleanTextForTTS(text: string): string {
  return text
    ...
    .replace(/([^\n\s\d])(\s*)(\d{1,2}[.)]) /g, '$1. $3 ')
    // 리스트 마커+숫자 패턴 ("9. 2018년", "27. 2026 IEEE" 등) → Azure TTS가 합쳐 읽음
    // 양쪽 숫자를 한글 표기로 명시 변환 → "구. 이천십팔년", "이십칠. 이천이십육 IEEE"
    // 매칭 조건: 숫자.공백+숫자 뒤에 비숫자 또는 문자열 끝 (한글/영어/공백/구두점 모두 OK)
    .replace(/(\d{1,3})\.\s+(\d{1,4})(?=\D|$)/g,
      (_, a, b) => `${toSinoKorean(a)}. ${toSinoKorean(b)}`)
    // 카운터 단위 앞 한자어 숫자 → 한국 고유어 숫자 변환 (TTS 자연스러움)
    // 예: "1대도" → "한 대도", "2명" → "두 명"
    // 조건: 앞에 다른 숫자 없음(11대 등 두자리는 제외), 뒤에 안전한 조사·구두점
    .replace(
      /(?<![0-9])([1-9])(대|명|마리|권|채|그루|송이|켤레|벌|잔|통)(?=$|[\s,.!?…。？！도만뿐의와과가을를이는에로])/g,
      (_, n, c) => {
