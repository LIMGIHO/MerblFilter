...
export default function TTSPlayer() {
  const { items, currentIndex, drawerOpen, remove, setCurrentIndex, clear, toggleDrawer, setDrawerOpen } =
    useTtsPlaylistStore();
  const { isSupported, status, rate, setRate, play, pause, resume, stop } = useTTS();

  // 본문 캐시: postId → 정제된 텍스트
  ...
  // 다음 트랙으로 이동 (참조용 — useEffect 의존성 회피 위해 ref에 저장)
  ...
  // 현재 아이템 재생 (재생 종료 시 onEnd 콜백으로 자동 다음 트랙)
  ...
  // currentIndex가 바뀌면 (자동 넘기기 포함) 즉시 재생
  ...
  function handleSelectItem(idx: number) {
    lastPlayedIndex.current = -1; // 강제 재생 허용
    setCurrentIndex(idx);
  }
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 flex flex-col items-center pointer-events-none">
      ...
      {/* 플레이어 바 */}
      <div className="pointer-events-auto w-full max-w-xl mx-auto mb-3 px-3">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl px-4 py-2.5 flex items-center gap-3">
          {/* 현재 트랙 정보 (클릭 시 드로어 토글) */}
          <button
